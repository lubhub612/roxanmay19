...
import { CrossIcon, StyledAnchor, StyleImageContainer, StyleImage, ActionOverlay, ActionIcon } from "./styles";
...
}, { name: "camera", Icon: AiFillCamera }, { name: "era", Icon: AiOutlineFieldTime }, { name: "themes", Icon: BiBrushAlt }, { name: "styles", Icon: FaShoppingBag },
{ name: "techniques", Icon: MdcHammerWrench }, { name: "lighting", Icon: AiOutlineBulb }, { name: "movies-games", Icon: MdcMovieSearchOutline }, { name: "architecture", Icon: BisBuildings }
    , { name: "fashion", Icon: GiLargeDress }, { name: "character", Icon: FaPersonBooth }, { name: "background", Icon: GiMountainCave }
]
const FiltersTab = ({ handleImageClick, handleRemove }) => {
    const { categories, styles, getStylesByCategory, searchStyleByKeyword, selectedStyles, setSelectedStyles } = useAiStylesContext();
    ...
    const handleImageTextWeightSubmission = (weight) => {
        const index = selectedStyles.findIndex(style => style._id === weight.styleId);
        if (index !== -1) {
    ...
    const handleStyleSelect = (style) => {
        const index = selectedStyles.findIndex(item => item.style_name === style.style_name);
        if (index === -1) {
    ...
    const handleRemoveImage = (style) => {
        setSelectedStyles(selectedStyles.filter(item => item.style_name !== style.style_name));
    ...
    const handleCategoryIcon = (categoryName) => {
        ...
        return <Icon className="bi bi-file-earmark-image" width="16"
            height="16" />
    }
    ...
    return (
        <Texttab>
            <input id="search-input" type="text" placeholder="Search Keyword..." onFocus={() => handleInputOnFocus()} value={searchKeyword} onChange={handleSearchInputField} />
            <ul>

                <li key="selected" className={-1 === selectedStyle && "active"} onClick={() => handleSelect({ index: -1 })}>
                    <StyledAnchor>
                ...
                </li>
                {
                    hideCategorySection
                    &&
                    <li key="selected" className={-2 === selectedStyle && "active"} onClick={() => handleSelect({ index: -2 })}>
...

                {
                    ...
                    && categories.map((item, index) =>
                        <li key={item._id} className={index === selectedStyle && "active"} onClick={() => handleSelect({ index, item })}>
                            <StyledAnchor>
                    ...
                    selectedStyle === -1
                        ...
                        && selectedStyles.map(style =>
                            <StyleImageHolder
                                ...
                                deleteAction={true}
                                handleImageTextWeightSubmission={handleImageTextWeightSubmission}
                            />
