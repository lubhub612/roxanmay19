...
export const AiStylesProvider = props => {
    ...
    const getStylesByCategory = async category_id => {
        ...
        invokeServer("get", `/api/ai_styles/styles?category_id=${category_id}`)
            .then(res => res.data.result === 1 && setStyles(res.data.category.styles))
    ...
    const searchStyleByKeyword = async keyword => {
        invokeServer("get", `/api/ai_styles/search?keyword=${keyword}`)
            .then(res => res.data.result === 1 && setStyles(res.data.styles[0].search_result))
    ...
    return <AiStylesContext.Provider
        value={{ selectedParams, setSelectedParams, categories, styles, getStylesByCategory, searchStyleByKeyword, generateImage, generatedImageUrl, selectedStyles, setSelectedStyles }}
    >
        {props.children}
